...
def classify_311_request(request_text: str) -> dict:
    ...
    # TODO: Step 1.2 - Call client.chat.completions.create()
          ...
          messages=[
              {
                "role": "system",
                "content": (
                    "You are a classifier for Memphis 311 service requests. "
                    ...
                    "  confidence: a float between 0.0 and 1.0 (not a percentage)\n"
...
def extract_key_phrases(text: str) -> list[str]:
    ...
    if response[0].is_error:
        raise RuntimeError(f"Key phrase extraction failed: {response[0].id}")
